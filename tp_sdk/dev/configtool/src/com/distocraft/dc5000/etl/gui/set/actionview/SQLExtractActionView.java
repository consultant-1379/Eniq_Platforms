...
public class SQLExtractActionView implements ActionView {
  ...
  public SQLExtractActionView(JPanel parent, Meta_transfer_actions action) {
    ...
    l_dateColName.setToolTipText("Defines the column where the data date is retrieved, if empty data date is the extraction date.");
    ...
    rowBufferSize.setToolTipText("Output file buffer size. How many rows is written into output file in one iteration.");
    JLabel l_rowBufferSize = new JLabel("Row Buffer Size");
    l_rowBufferSize.setToolTipText("Output file buffer size. How many rows is written into output file in one iteration.");
