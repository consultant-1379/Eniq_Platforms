...
import com.distocraft.dc5000.etl.gui.set.AlarmInterfaceWizardDialog;
import com.distocraft.dc5000.etl.rock.Meta_collection_sets;
import com.distocraft.dc5000.etl.rock.Meta_collections;
import com.distocraft.dc5000.etl.rock.Meta_transfer_actions;

/**
 * This class creates the Directory Checker set for the Alarm Interface Wizard. It creates the Directory Checker for the "Alarminterfaces" package.
 * @author berggren
 *
 */
public class CreateAlarmDirectoryChecker {

  private Logger log = Logger.getLogger("CreateAlarmDirectoryChecker.create");
   ...
   * @param interfaceName Name of the interface the wizard is creating.
   * @param rockFactory Rockfactory for database operations.
   * @param alarmInterfacesPackage The "Alarminterfaces" package where this Directory Checker is to be created.
   * @param frame is a JFrame object for showing error window.
   */
  ...
  public void create() {
    ...
    actionContents = new Properties();
    actionContents.setProperty("permission", "770");
    createAction("CreateDir_" + this.interfaceName + "_out", new Long(actionOrder), baseDirectory + this.interfaceName
        + "/" + "out" + "/", actionContents, "Y", new Long(2));
    ...
    actionContents = new Properties();
    actionContents.setProperty("permission", "770");
    createAction("CreateDir_" + etlDataDirectory + "adapter_tmp" +"/" + this.interfaceName + "/", new Long(actionOrder),
        etlDataDirectory + "adapter_tmp" + "/" + this.interfaceName + "/", actionContents, "Y", new Long(2));
  ...
  private void createDirectoryCheckerSet() {
    ...
    try {
      ...
      this.directoryCheckerSet.setCollection_id(AlarmInterfaceWizardDialog.getNewCollectionId(this.rockFactory,this.frame));
   ...
   * @param transferActionName Name of the action.
   * @param actionOrder Execution order of the action.
   * @param whereClause WhereClause of the action.
   * @param actionContents ActionContents of the action.
   * @param enabledFlag EnabledFlag od the action.
   * @param connectionId ConnectionId of the action.
   */
  private void createAction(String transferActionName, Long actionOrder, String whereClause, Properties actionContents,
      String enabledFlag, Long connectionId) {
    ...
    targetAction.setAction_contents(propertyToString(actionContents));
    targetAction.setEnabled_flag(enabledFlag);
    targetAction.setConnection_id(connectionId);
    targetAction.setTransfer_action_id(AlarmInterfaceWizardDialog.getNewCollectionId(this.rockFactory,this.frame));
  ...
  /**
   * This function transforms a Properties object to a String.
   * @param properties is the Properties object.
   * @return Returns String-object containing the Properties.
   * @throws Exception
   */
  protected String propertyToString(Properties properties) {
    try {
      ...
      this.log.log(Level.SEVERE, "propertyToString conversion failed.", e);
      return "";
    }
