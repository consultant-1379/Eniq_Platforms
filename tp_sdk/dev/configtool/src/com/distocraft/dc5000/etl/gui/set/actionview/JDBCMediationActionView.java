...
public class JDBCMediationActionView implements ActionView {
  ...
  public JDBCMediationActionView(JPanel parent, Meta_transfer_actions action) {
    ...
    drv.setText(orig.getProperty("jdbc_drv", "com.sybase.jdbc3.jdbc.SybDriver"));

    JLabel l_drv = new JLabel("JDBC Driverclass");
    l_drv.setToolTipText("JDBC driver class name. The jar containing the class must be available in ETLC server classpath.");
    parent.add(l_drv, c);

    c.weightx = 1;
    c.gridx = 1;
    drv.setToolTipText("JDBC driver class name. The jar containing the class must be available in ETLC server classpath.");
    parent.add(drv, c);
    ...
    l_tempDir.setToolTipText("The directory where incomplete output files are temporarily stored. When complete files are moved into Ouput Directory.");
    parent.add(l_tempDir, c);

    c.weightx = 1;
    c.gridx = 1;
    tempDir.setToolTipText("The directory where incomplete output files are temporarily stored. When complete files are moved into Ouput Directory.");
