...
Public Class UnivJoins
    ...
    Public Sub getJoins(ByRef tp_name As String, ByRef mts As MeasurementTypes, ByRef conn As System.Data.OleDb.OleDbConnection, ByRef dbCommand As System.Data.OleDb.OleDbCommand, ByRef dbReader As System.Data.OleDb.OleDbDataReader)
        ...
        While (dbReader.Read())
            ...
            Else
                If dbReader.GetValue(2).ToString() <> "" Then
                    StartT = dbReader.GetValue(0).ToString()
                    StartL = dbReader.GetValue(1).ToString()
                    StartC = dbReader.GetValue(2).ToString()
                    EndT = dbReader.GetValue(3).ToString()
                    EndL = dbReader.GetValue(4).ToString()
                    EndC = dbReader.GetValue(5).ToString()
                    Cardinality = dbReader.GetValue(6).ToString()
                    Contexts = dbReader.GetValue(7).ToString()
                        ...
                        For count = 1 To mts.Count
                            ...
                            If mt.RankTable = True AndAlso mt.ObjectBusyHours <> "" Then
                                ...
                                For bh_count = 1 To bh_mts.Count
                                    ...
                                    If bh_mt.RankTable = False AndAlso bh_mt.ObjectBusyHours = mt.ObjectBusyHours Then
                                        If First = True Then
                                    ...
                                    End If
                                Next bh_count
                                StartT = Replace(StartT, "(OBJECTBH_MT)", bhtables)
                                EndT = Replace(EndT, "(DIM_OBJECTRANKMT)", Replace(mt.MeasurementTypeID, "DC_", "DIM_"))
                                makeJoins(tp_name, StartT, StartL, StartC, EndT, EndL, EndC, Cardinality, Contexts)
                            End If
                        Next count
                    ElseIf InStrRev(dbReader.GetValue(0).ToString(), "(ELEMENTRANKMT)") > 0 Then
                        For count = 1 To mts.Count
                            ...
                            If mt.RankTable = True AndAlso mt.ElementBusyHours = True Then
                                StartT = Replace(StartT, "(ELEMENTRANKMT)", mt.MeasurementTypeID)
                                EndT = Replace(EndT, "(DIM_ELEMENTRANKMT)", Replace(mt.MeasurementTypeID, "DC_", "DIM_"))
                                makeJoins(tp_name, StartT, StartL, StartC, EndT, EndL, EndC, Cardinality, Contexts)
                            End If
                        Next count
                        ...
                        For count = 1 To mts.Count
                            ...
                            If mt.RankTable = True AndAlso mt.ElementBusyHours = True Then
                                ...
                                For bh_count = 1 To bh_mts.Count
                                    ...
                                    End If
                                Next bh_count
                                StartT = Replace(StartT, "(ELEMENTBH_MT)", bhtables)
                                EndT = Replace(EndT, "(ELEMENTRANKMT)", mt.MeasurementTypeID)
                                StartC = Replace(StartC, "(ELEMENTCOLUMN)", element_column)
                                EndC = Replace(EndC, "(ELEMENTCOLUMN)", element_column)
                                makeJoins(tp_name, StartT, StartL, StartC, EndT, EndL, EndC, Cardinality, Contexts)
                            End If
                        Next count
                    Else
    ...
    Public Sub makeJoins(ByRef tp_name As String, ByRef StartTables As String, ByRef StartLevels As String, ByRef StartColumns As String, ByRef EndTables As String, ByRef EndLevels As String, ByRef EndColumns As String, ByRef Cardinality As String, ByRef Contexts As String)
        ...
        For count = 1 To mts.Count
            ...
            For cnt_count = 1 To cnts.Count
                ...
                If cnt.CounterType = "VECTOR" Then
                    ...
                    CreateJoinExpression("DC." & cnt.MeasurementTypeID, "All", cnt.CounterName & "_DCRNG", "DC." & VectorTable, "", cnt.CounterName & "_DCRNG")
                    For JoinCount = 0 To UBound(JoinsArray)
                        Dim univ_join = New UnivJoins.UnivJoin
                        univ_join.Expression = JoinsArray(JoinCount)
    ...
    Public Sub buildLists(ByRef mts As MeasurementTypes)
        ...
        ReDim Preserve measurements(300)
    ...
    Private Function CreateJoinExpression(ByRef StartTables As String, ByRef StartLevels As String, ByRef StartColumns As String, ByRef EndTables As String, ByRef EndLevels As String, ByRef EndColumns As String) As Object
        ...
        ReDim Preserve JoinStartTableLevels(300)
        start_table_count = 0
