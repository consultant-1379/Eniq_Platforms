...
public class DefaultAlarmcfgSession implements IAlarmcfgSession {
	...
	public IAlarmcfgReport getReport(final IAlarmcfgReportRequest request) throws ACSessionException,
		...
		try {
			...
			throw new ACSessionException("Couldn't get URL: '" + reportURL + " (" + e.getMessage() + ")");
		} catch (final ACReportNotFoundException e) {
			throw new ACReportNotFoundException("Report not found: " + e.getMessage());
		} catch (final Exception e) {
			throw new ACSessionException("Fatal error when getting URL: '" + reportURL + " (" + e.getMessage() + ")");
		}
