#!/bin/bash
#----------------------------------------------------------------------
# Ericsson Network IQ procus_simcfg installation script
#
...
LOGFILE=${LOG_DIR}/platform_installer/procus_simcfg_${TIMESTAMP}.log
...
VTAG="module.procus_simcfg=${VER}b${BLD}"
...
if [ ! -f ${INSTALLER_DIR}/versiondb.properties ] ; then
  ...
  OLD=`$CAT ${INSTALLER_DIR}/versiondb.properties | $GREP module.procus_simcfg`
  if [ -z "${OLD}" ] ; then
    $ECHO "${VTAG}" >> ${INSTALLER_DIR}/versiondb.properties
  else
...
if [ $VERBOSE = 1 ] ; then
  $ECHO "procus_simcfg installed" | $TEE -a ${LOGFILE}
fi
