#!/bin/bash
#----------------------------------------------------------------------
# Ericsson Network IQ simcfg_procus installation script
#
...
LOGFILE=${LOG_DIR}/platform_installer/simcfg_procus_${TIMESTAMP}.log
...
VTAG="module.simcfg_procus=${VER}b${BLD}"
...
if [ ! -f ${INSTALLER_DIR}/versiondb.properties ] ; then
  ...
  OLD=`$CAT ${INSTALLER_DIR}/versiondb.properties | $GREP module.simcfg_procus`
  if [ -z "${OLD}" ] ; then
    $ECHO "${VTAG}" >> ${INSTALLER_DIR}/versiondb.properties
  else
...
if [ $VERBOSE = 1 ] ; then
  $ECHO "simcfg_procus installed" | $TEE -a ${LOGFILE}
fi
