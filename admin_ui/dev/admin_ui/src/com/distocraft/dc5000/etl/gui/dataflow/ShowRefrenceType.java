...
public class ShowRefrenceType extends EtlguiServlet {
   ...
   */
  private Vector<Vector> getReferenceTypeTable(Context ctx, String type, int rowLimit, String filtercolumn, String filtertype,
      String filtervalue) throws SQLException {
    Statement stmt = null;
    ResultSet rset = null;
    Vector<Vector> retVal = new Vector<Vector>();
    ...
    try {
      ...
      while (rset.next()) {
			...
			for (int i = 1; i < j; i++) {
				if (i == latitude) {
					float value = Float.parseFloat(rset.getString(i));
					if (value < 0) {
						result = conValue(value, "S");
						v.add(result);
					} else {
						result = conValue(value, "N");
						v.add(result);
					}
				} else if (i == longitude) {
					float value = Float.parseFloat(rset.getString(i));
					if (value < 0) {
						result = conValue(value, "W");
						v.add(result);
					} else {
						result = conValue(value, "E");
						v.add(result);
					}
				} else {
