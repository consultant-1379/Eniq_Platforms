...
public class ETLSessionLog extends EtlguiServlet { // NOPMD by eheijun on 02/06/11 15:09
  ...
  private List<List<String>> getEtlAdapterRowDetails(final Context ctx, final String batch_id, final String session_id) {
    final String sql = "select * from log_session_loader where session_id=" + session_id + " " + " and batch_id=" + batch_id
        + "";
  ...
  private List<List<String>> getMZLoggerRowDetails(final Context ctx, final String batch_id, final String session_id, String loggerx,
    ...
    final String sql = "select " + selCols + "  from log_session_adapter where session_id=" + session_id + " "
        + " and batch_id=" + batch_id + " and WORKFLOW_TYPE ='" + loggerx + "' and WORKFLOW_NAME LIKE '%" + wfName
        + "%'";
  ...
  private List<List<String>> getEtlLoggerRowDetails(final Context ctx, final String batch_id, final String session_id) {
    ...
    final String sql = "select distinct FileName, SessionEndTime, SessionStartTime, Source, Status from log_session_adapter where session_id="
        + session_id + " " + " and batch_id=" + batch_id + "";
  ...
  private void getInformation(final Context ctx, final String year_1, final String month_1, final String day_1, final String logger, final String tpack, // NOPMD by eheijun on 02/06/11 15:09
    ...
    if (logger.equals("0")) {
      ...
      // tpackfilter = "' and source LIKE '%" + tpack + "%'";
      tpackfilter = " and filename LIKE '%" + a_filename + "%' and source LIKE '%" + a_source + "%' and status='"
          + a_status + "' ";
      ...
      stime = "datetime(sessionendtime) between datetime('" + dataTime + " " + starthour + ":00:00') and  "
          + "datetime('" + dataTime2 + " " + endhour + ":00:00')";
    } else if (logger.equals("1")) {
      table = "LOG_SESSION_LOADER";
      tpackfilter = " and typename LIKE '%" + tpack + "%' and source LIKE '%" + source + "%' ";
      stime = " (datetime(sessionendtime) between datetime('" + dataTime + " " + starthour + ":00:00') and  "
          + "datetime('" + dataTime2 + " " + endhour + ":00:00'))";
      ...
      // tpackfilter = "' and source LIKE '%" + tpack + "%'";
      tpackfilter = " and typename LIKE '%" + tpack + "%'";
      stime = "datetime(sessionendtime) between datetime('" + dataTime + " " + starthour + ":00:00') and  "
          + "datetime('" + dataTime2 + " " + endhour + ":00:00')";
    } else if (logger.equals("3")) {
      table = "LOG_SESSION_ADAPTER";
      tpackfilter = " and WORKFLOW_TYPE = '" + Helper.MZ_PROCESSING + "' ";
      ...
      if (a_filename != null) {
        tpackfilter += " and filename LIKE '%" + a_filename + "%' ";
        log.debug("3-a_filename=" + a_filename);
      }
      if (a_source != null) {
        tpackfilter += "and source LIKE '%" + a_source + "%' ";
        log.debug("3-a_source=" + a_source);
      }
      if (a_status != null) {
        tpackfilter += " and status='" + a_status + "' ";
      ...
      tpackfilter = " and WORKFLOW_TYPE = '" + Helper.MZ_PREPROCESSING + "' ";
    ...
    String sqlStr = "select top " + maxrows + " * " + "from " + table + " where " + stime + tpackfilter;
    ...
    if (table.equalsIgnoreCase("LOG_SESSION_ADAPTER")) {
      ...
      sqlStr = "select distinct top " + maxrows + selectColumns + "from " + table + " where " + stime + tpackfilter;
    }
    ...
    try {
      ...
      ctx.put("results", v);
      ctx.put("toomany", v.size() >= Integer.parseInt(maxrows) ? maxrows : " ");
    } catch (SQLException e) {
      log.info("SQLException: " + e);
