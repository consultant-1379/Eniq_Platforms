 ...
 *
 * Initialises database connection objects and places them in context for sub classes.
 *
...
public abstract class EtlguiServlet extends GuiServlet {
  ...
  private static final int SESSION_TIMEOUT = 3 * 60 * 60; // 3 Hours
  ...
  protected Properties loadConfiguration(final ServletConfig config) throws IOException, FileNotFoundException {
    ...
    p.setProperty(Velocity.FILE_RESOURCE_LOADER_PATH, getWebRootPath(config) + WEB_INF_TEMPLATES);
  ...
  @Override
  public final Template handleRequest(final HttpServletRequest request, final HttpServletResponse response,
    ...
    try {
      ...
      try{
    	  rockFactoryTypeArray = DbConnectionFactory.getInstance().initialiseConnections(session);
      }catch(Exception e){
      ...
      log.debug("doHandleRequest done.");
    	  ...
    	  try{
    		  ...
    		  if(ENIQServiceStatusInfo.isEtlDBOffline()){
    			  context.put("errorText", " Failed to initialize connection to database: " + ENIQServiceStatusInfo.getEtlDBName());
    		  }else if(ENIQServiceStatusInfo.isRepDBOffline()){
    			  context.put("errorText", " Failed to initialize connection to database: " + ENIQServiceStatusInfo.getRepDBName());
    		  }else if(ENIQServiceStatusInfo.isDwhDBOffline()){
    			  context.put("errorText", " Failed to initialize connection to database: " + ENIQServiceStatusInfo.getDwhDBName());
    		  }else{
    		  ...
    		  log.error("Exception getting velocity template: " + ADMINUI_NEW_ERROR_PAGE_TEMPLATE_WITHMENU + " ", e1);
    	  }
    }
    return viewTemplate;
  }
  ...
  /**
   * Template method pattern. Subclasses of this servlet implement this method.
   *
   * @param request
   * @param response
   * @param ctx
   ...
   */
  public abstract Template doHandleRequest(HttpServletRequest request, HttpServletResponse response, Context ctx) throws Exception;  // NOPMD by eheijun on 02/06/11 14:27
