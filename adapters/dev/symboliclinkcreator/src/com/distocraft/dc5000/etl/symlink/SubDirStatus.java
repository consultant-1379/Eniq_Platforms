...
public class SubDirStatus {
     ...
     */
    SubDirStatus(final EniqSymbolicLinkFactory symbolicLinkFactory, final int numberOfLinks, final int subDirNumber) {
            ...
            if (symbolicLinkFactory.symLinkDirectoryRule == EniqSymbolicLinkFactory.MAX_PER_DIRECTORY) {
                ...
                if (numberOfLinks > symbolicLinkSubDirConfiguration.getMaxNumLinks()) {
                    ...
                    if (++numberOfFoldersTried > symbolicLinkSubDirConfiguration.getSubDirs().size()) {
                        handleAllDirectoriesFull(symbolicLinkSubDirConfiguration);
                    }
                }
            } else {
    ...
    /**
     * Handles the situation when all the sub directories for a NeType are 
     * filled with maximum number of allowed symbolic links.
     * 
     * @param symbolicLinkSubDirConfiguration
     * @param currentDir
     * @param directory
     * @throws SymbolicLinkCreationFailedException
     */
    private void handleAllDirectoriesFull(final SymbolicLinkSubDirConfiguration symbolicLinkSubDirConfiguration)
            throws SymbolicLinkCreationFailedException {
        throw new SymbolicLinkCreationFailedException("All sub directories have reached their maximum limit."
                + symbolicLinkSubDirConfiguration);
    }
