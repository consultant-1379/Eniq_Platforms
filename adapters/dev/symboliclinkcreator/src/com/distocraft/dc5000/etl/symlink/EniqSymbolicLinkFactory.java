...
public class EniqSymbolicLinkFactory extends AbstractSymbolicLinkFactory {
	...
	*/
	...
	@Override
	SymbolicLinkSubDirConfiguration getSymbolicLinkSubDirConfiguration(final String nodeType) {
	...
	String getLinkSubDir(
			...
			String nodeName, List<String> depSpecificSubDirList, String depName)
	...
	String getLinkSubDir(
		...
		final String nodeName) throws SymbolicLinkCreationFailedException {
	...
	synchronized (subDirStatus) {
		...
		if (subDirStatus.numberOfLinks > symbolicLinkSubDirConfiguration
				.getMaxNumLinks()) {
			System.out.println("Reached max no links for directory number "+ subDirStatus.subDirNumber);
	...
	*/
	...
	if (currentSubDirStatus == null) {
		currentSubDirStatus = getSubDirStatus();
	...
	* @SubDirStatus object
	*/
	protected SubDirStatus getSubDirStatus() {
	return new SubDirStatus(this, 0, 0);
	}
