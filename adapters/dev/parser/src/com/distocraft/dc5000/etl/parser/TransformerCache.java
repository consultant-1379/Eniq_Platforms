...
public class TransformerCache {
  ...
  public static void setCheckTransformations(boolean checkTransformations) {
	TransformerCache.checkTransformations = checkTransformations;
  }
  ...
  public Transformer getTransformer(final String transformerID) {
    ...
    Iterator keys = null;
    for(int i=0;i<5;i++){
    	t= (Transformer) transformers.get(transformerID);

    if (t != null){
      return t;
    }
    if (isCheckTransformations()){
    	try {
    		log.fine("Retrying "+i+" to refresh transformation cache for :"+transformerID);
    ...
    if (t == null) {
        log.finest("Cache miss or no such transformer \"" + transformerID + "\"");
...
//        if (techPacks != null && (!techPacks.isEmpty() && !techPacks.contains(tpa.getTechpack_name()))) {
//          System.out.println("Skipping " + tpa.getTechpack_name());
//          continue;
//        }
//        System.out.println("Revalidating " + tpa.getTechpack_name());
   ...
   *
   * @param tpName     Name of the techpack to revalidate.
   * @param dwhreprock RockFactory to dwhrep.
   * @param dwhrock    RockFactory to dwh.
   */
