...
public class SourceFileTest {
  ...
  @Test
  public void testCounterVolumes_DifferentTimestamp(){

    final SimpleDateFormat dataFormatter = new SimpleDateFormat("yyyy-MM-dd kk:mm:ss");

    final String mType = "DC_E_RAN_UCELL_V";
    final String dateTimeId1 = dataFormatter.format(new Date());
    final String dateTimeId2 = dataFormatter.format(new Date(System.currentTimeMillis() - 60000));

    final MeasurementFile mFile1 = mockMeasurementFile(mType, dateTimeId1, 2, 6);
    final MeasurementFile mFile2 = mockMeasurementFile(mType, dateTimeId2, 3, 10);

    objUnderTest.setErrorMessage(null);
    objUnderTest.addMeasurementFile(mFile1);
    objUnderTest.addMeasurementFile(mFile2);

    final List<String> expectedKeys = Arrays.asList(mType + "_" + dateTimeId1, mType + "_" + dateTimeId2);
    final Map<String, Integer> expectedRows = new HashMap<String, Integer>();
    expectedRows.put(mType + "_" + dateTimeId1, 2);
    expectedRows.put(mType + "_" + dateTimeId2, 3);
    final Map<String, Long> expectedCounters = new HashMap<String, Long>();
    expectedCounters.put(mType + "_" + dateTimeId1, 6L);
    expectedCounters.put(mType + "_" + dateTimeId2, 10L);

    assertCounterVolume(objUnderTest, expectedKeys, expectedRows, expectedCounters);
  }

  @Test
  public void testCounterVolumes_SameTimestamp(){

    final SimpleDateFormat dataFormatter = new SimpleDateFormat("yyyy-MM-dd kk:mm:ss");

    final String mType = "DC_E_RAN_UCELL_V";
    final String dateTimeId = dataFormatter.format(new Date());
    ...
    final Map<String, Integer> expectedRows = new HashMap<String, Integer>();
    expectedRows.put(mType + "_" + dateTimeId, 5);
    final Map<String, Long> expectedCounters = new HashMap<String, Long>();
    expectedCounters.put(mType + "_" + dateTimeId, 16L);

    assertCounterVolume(objUnderTest, expectedKeys, expectedRows, expectedCounters);
  }

  private void assertCounterVolume(final SourceFile sourceFile, final List<String> expectedKeys,
                                   final Map<String, Integer> expectedRows,
                                   final Map<String, Long> expectedCounters) {
    final Map sessionLog = sourceFile.getSessionLog();
    assertTrue("No counter volume data stored in session log entry", sessionLog.containsKey("counterVolumes"));
    final Map<String, Map<String, String>> counterVolume = (Map<String, Map<String, String>>) sessionLog.get("counterVolumes");

    for (String expectedKey : expectedKeys) {
      assertTrue("No counter volume info found for " + expectedKey, counterVolume.containsKey(expectedKey));
      final Map<String, String> volumeData = counterVolume.get(expectedKey);

      assertTrue("No rowCount key found in volume info", volumeData.containsKey("rowCount"));
      assertTrue("No counterVolume key found in volume info", volumeData.containsKey("counterVolume"));

      final int actualRowCount = Integer.valueOf(volumeData.get("rowCount"));
      final long actualCounterCount = Long.valueOf(volumeData.get("counterVolume"));

      final int exptectedRowCount = expectedRows.get(expectedKey);
      final long exptectedCounterCount = expectedCounters.get(expectedKey);

      assertEquals("Incorrect rowCount value stored in session info!", exptectedRowCount, actualRowCount);
      assertEquals("Incorrect counterVolume value stored in session info!", exptectedCounterCount, actualCounterCount);
    }
  }

  private MeasurementFile mockMeasurementFile(final String typeName, final String dateTimeId,
                                              final int rows, final long counters){
    final MeasurementFile mFile = EasyMock.createNiceMock(MeasurementFile.class);
  ...
  public void testHide() throws Exception {
	...
	final String hiddenDirName = ".HIDDEN";
	final File hiddenDir = new File(physicalSourceFile.getParent()+File.separator+hiddenDirName);
	  
	/* Defining what the result should be */
	final File hiddenFile = new File(hiddenDir.getPath()+File.separator+physicalSourceFile.getName());
	
    /* Calling the tested method and asserting file is hidden */
    objUnderTest.hide(hiddenDirName);
    assertEquals(false, physicalSourceFile.exists());
   ...
   */
  @Test
  public void testHide_dirAlreadyExists() throws Exception {
  ...
  //public void testMove() throws Exception {
	...
	/* Defining what the result should be */
	//final File movedFile = new File(physicalSourceFile.getParent()+"/tmp_sym_link_dir/"+physicalSourceFile.getPath());
