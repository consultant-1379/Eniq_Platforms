...
/**
 * MeasurementFile of sim. Takes parsed data in by addData methods and writes default form output file. <br>
...
public class MeasurementFileImpl implements MeasurementFile {
     ...
     * @throws Exception
     */
    MeasurementFileImpl(final SourceFile sfile, final String tagID, final String techPack, final String set_type, final String set_name,
     ...
     */
    MeasurementFileImpl(final SourceFile sfile, final String tagID, final String techPack, final String set_type, final String set_name,
                        final String workerName, final Logger log) throws Exception {
        ...
        // Find out the format of the output from sim (e.g. ASCII or binary).
        String outputFormatTemp = sf.getProperty("outputFormat", null);
        simType = sf.getProperty("simType", null);
        if (simType != null && null == outputFormatTemp) {
            if (simType.equalsIgnoreCase("MDC")) {
                outputFormatTemp = sf.getProperty("MDCSim.outputFormat", "0");
            } else if (simType.equalsIgnoreCase("3gpp32435")) {
                outputFormatTemp = sf.getProperty("x3GPPSim.outputFormat", "0");
            } else {
                outputFormatTemp = "0";
            }
            log.finest("Sim Type is " + simType);
            log.finest("Output Format is " + outputFormatTemp);
        } else {
            // Check output format
            log.finest("Sim Type :: " + simType);
        ...
        if (b) {
            this.debugger = SimDebuggerCache.getCache();
        }
    ...
    private Charset getOutputCharsetEncoding() {
        ...
        try {
            ...
            log.log(Level.WARNING, "Cannot instantiate charset object out of the database charset setting: " + dwhdbCharsetEncoding, e);
    ...
    @Override
    public void setDebugger(final SimDebugger debugger) {
    ...
    public void saveData() throws Exception {
        ...
        if (writer == null && bwriter == null) {
		  ...
		  // Ignore metadata = We create metadata based on the first dataline
            ...
            /*//Fix for TR HS49435 //Fix for TR HS86464 & HS83771
             if (((data.get("OSS_ID") == null) || (data.get("OSS_ID").length() == 0)) && !((techpack.contains("IPTNMS")) || (techpack.contains("SOEM"))))
		    {
            	
      			log.info("OSS_ID ::"+data.get("OSS_ID"));
      			if(flag)
      			{
        ...
        while (iterator.hasNext()) {
            ...
            if (outputFormat == IQ_TEXT) {
                ...
                if (bf == null) {
                	...
                	log.warning("JVM cache is not updated properly, So refreshing Transformations");
                	refreshTransformationCache();
                     ...
                     */
                    // FIX for TR HS14474
                    //throw new Exception("JVM cache is not updated properly , Restart the services again");
                ...
                try { // Convert to binary form
                    ...
                    log.warning("Could not convert data item " + mData.getDataName() + " (of " + dataformat.getTagID() + ") to binary form. Value: "
        ...
        data.clear();
        log.finer("Measurement File saved data for " + getTagID() + " in worker{" + workerName + "}" + " with Rows{" + getRowCount() + "} Counters{"
                + getCounterVolume() + "}");

    }
    
    private synchronized void refreshTransformationCache() throws  MalformedURLException, RemoteException, NotBoundException {
    ...
    public void close() throws Exception {
        ...
        if (data != null) {
            ...
            log.finer("Measurement File being closed for data " + getTagID() + " in worker{" + workerName + "}" + " with Rows{" + getRowCount()
                    + "} Counters{" + getCounterVolume() + "}");
            data.clear();
        }
    ...
    public String toString() {
        return "MeasurementFile " + outputFileName + " type " + typeName + " TAGID " + this.tagID + " Techpack " + this.techpack;
    ...
    private String determineOutputFileName(final String tagid, final String typeName) throws Exception {
        final String eventsEtlDataDir = System.getProperty(CommonUtils.EVENTS_ETLDATA_DIR, CommonUtils.EVENTS_ETLDATA_DIR_VAL);
    ...
    private String createExpandedDirs(final String eventsEtlDataDir, final int folderNumber, final String[] splitStrs) throws Exception {
