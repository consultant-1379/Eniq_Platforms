 ...
 * <code>TableColumnModelListener</code>. Therefore hiding a column will
 * result in <code>columnRemoved</code> event and showing it again will notify
...
public class XTableColumnModel extends DefaultTableColumnModel {
	...
	public void setAllColumnsVisible() {
		...
		for (int columnIndex = 0; columnIndex < noColumns; ++columnIndex) {
			TableColumn visibleColumn = (columnIndex < tableColumns.size() ? (TableColumn) tableColumns
					.get(columnIndex)
					: null);
			TableColumn invisibleColumn = (TableColumn) allTableColumns
					.get(columnIndex);
	...
	public TableColumn getColumnByModelIndex(int modelColumnIndex) {
		for (int columnIndex = 0; columnIndex < allTableColumns.size(); ++columnIndex) {
			TableColumn column = (TableColumn) allTableColumns
	 ...
	 * <code>columnRemoved</code> event. Will do nothing if the column is not
	 * in this model.
	 * 
	 * @param column
	 *            the column to be added
	 ...
	 * <code>identifier</code>. Position is the the index in all visible
	 * columns if <code>onlyVisible</code> is true or else the index in all
	 * columns.
	 * 
	 * @param identifier
	 ...
	 * 
	 * @exception IllegalArgumentException
	 *                if <code>identifier</code> is <code>null</code>, or
	 *                if no <code>TableColumn</code> has this
	 *                <code>identifier</code>
	 * @see #getColumn
	 */
