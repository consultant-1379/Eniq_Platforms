...
import com.ericsson.eniq.techpacksdk.view.verification.VerificationConditionTableModel;

@SuppressWarnings("serial")
public class ManageVerificationConditionView extends JPanel {

	private static final Logger logger = Logger
			.getLogger(ManageVerificationConditionView.class.getName());
	...
	private VerificationConditionDataModel verificationConditionDataModel;

	private JTable datatable;

	private VerificationConditionTableModel verificationConditionTableModel;

	private List<Verificationcondition> data;

	private List<Verificationcondition> tobedeleted;

	private class VerificationconditionTableListener implements
			TableModelListener {
	...
	private class SaveTask extends Task<Void, Void> {
		...
		protected Void doInBackground() throws Exception {
			try {
				dataModelController.getRockFactory().getConnection()
						.setAutoCommit(false);
				while (tobedeleted.size() > 0) {
					Verificationcondition vo = tobedeleted.remove(0);
					vo.deleteDB();
				}
				verificationConditionTableModel.save();
        
				((GeneralTechPackTab) parentPanel).setTPActivationModified(TPActivationModifiedEnum.MODIFIED_OTHER);

        dataModelController.rockObjectsModified(dataModelController
						.getVerificationConditionDataModel());
	...
	public Task<Void, Void> discard() {
		...
		frame.setGlassPane(busyIndicator);
		discardTask.setInputBlocker(new BusyIndicatorInputBlocker(discardTask,
				busyIndicator));
	...
	/**
	 * Add new verification condition action
	 */
	@Action(enabledProperty = "addEnabled")
	public void addemptyrow() {
		Verificationcondition newvc = new Verificationcondition(
				dataModelController.getRockFactory());
		newvc.setVersionid(versioning.getVersionid());
		newvc.setFacttable("ALL");
		newvc.setVerlevel(Constants.REPORTCONDITIONLEVELS[0]);
		newvc.setConditionclass("");
		newvc.setVercondition("");
		newvc.setPromptname1("");
		newvc.setPromptvalue1("");
		newvc.setPromptname2("");
		newvc.setPromptvalue2("");
		newvc.setPromptname3("");
		newvc.setPromptvalue3("");
		verificationConditionTableModel.addRow(newvc);
		fireTableDataChanged(true);
	}

	/**
	 * Remove selected verification condition action
	 */
	@Action(enabledProperty = "removeEnabled")
	public void removerow() {
		final int ix = datatable.getSelectionModel().getMinSelectionIndex();
		if (ix >= 0) {
			tobedeleted.add(verificationConditionTableModel.removeRow(ix));
		}
		fireTableDataChanged(true);
	}

	/**
	 * Duplicate selected verification condition action
	 */
	@Action(enabledProperty = "removeEnabled")
	public void duplicaterow() {
		...
		if (row >= 0) {
			// Get the source Verification Condition
			Verificationcondition oldCondition = verificationConditionTableModel
					.getRow(row);

			// Create the new Verification Condition
			Verificationcondition newCondition = new Verificationcondition(
					dataModelController.getRockFactory());

			// Copy the data
			newCondition.setVersionid(oldCondition.getVersionid());
			newCondition.setFacttable(oldCondition.getFacttable());
			newCondition.setVerlevel(oldCondition.getVerlevel());
			newCondition.setConditionclass(oldCondition.getConditionclass());
			newCondition.setVercondition(oldCondition.getVercondition());
			newCondition.setPromptname1(oldCondition.getPromptname1());
			newCondition.setPromptvalue1(oldCondition.getPromptvalue1());
			newCondition.setPromptname2(oldCondition.getPromptname2());
			newCondition.setPromptvalue2(oldCondition.getPromptvalue2());
			newCondition.setPromptname3(oldCondition.getPromptname3());
			newCondition.setPromptvalue3(oldCondition.getPromptvalue3());
			newCondition.setObjectcondition(oldCondition.getObjectcondition());

			// Add the new object to the table model
			verificationConditionTableModel.addRow(newCondition);
	...
	private javax.swing.Action getParentAction(final String actionName) {
		if (application != null) {
			return application.getContext().getActionMap(parentPanel).get(
					actionName);
		}
		return null;
	}
	...
	private void refresh() {
		verificationConditionDataModel.refresh();
		data = new Vector<Verificationcondition>(verificationConditionDataModel
				.getVerificationconditionData());
		verificationConditionTableModel.setData(data);
		fireTableDataChanged(false);
	}

	public ManageVerificationConditionView(
			final SingleFrameApplication application,
			DataModelController dataModelController, Versioning versioning,
		...
		verificationConditionDataModel = dataModelController
				.getVerificationConditionDataModel();

		verificationConditionDataModel.setCurrentVersioning(versioning);
		verificationConditionDataModel.refresh();

		data = new Vector<Verificationcondition>(verificationConditionDataModel
				.getVerificationconditionData());
		tobedeleted = new ArrayList<Verificationcondition>();

		MouseListener ml = new VerificationConditionTableSelectionListener();

		verificationConditionTableModel = new VerificationConditionTableModel(
				application, dataModelController, data);

		datatable = new JTable(verificationConditionTableModel);
		if (editable) {
		...
		datatable.getModel().addTableModelListener(
				new VerificationconditionTableListener());
		...
		verificationConditionTableModel.setColumnWidths(datatable);
		verificationConditionTableModel.setColumnEditors(datatable);
		verificationConditionTableModel.setColumnRenderers(datatable);
	...
	private void fireTableDataChanged(boolean newvalue) {
		Vector<String> errorStrings = verificationConditionTableModel
				.validateData();
	...
	private class VerificationConditionTableSelectionListener implements
			MouseListener {

		/**
     * 
     */
		public VerificationConditionTableSelectionListener() {
			super();
		}
