...
import com.ericsson.eniq.component.NumericDecimalDocument;
...
public class UpdateMonitoringActionView implements ActionView {
  ...
  public UpdateMonitoringActionView(JPanel parent, Meta_transfer_actions action, JDialog pdialog) {
    ...
    threshold = new JTextField(new NumericDecimalDocument(), orig.getProperty("threshold", ""), 5);
    threshold
        .setToolTipText("Time, in hours, that is waited before aggregations may start if data in incomplete. Default: 1 (hours)");

    JLabel th_l = new JLabel("Aggregation threshold");
    th_l
        .setToolTipText("Time, in hours, that is waited before aggregations may start if data is incomplete. Default: 1 (hours)");
    parent.add(th_l, c);
    ...
    completeThreshold = new JTextField(new NumericDecimalDocument(), orig.getProperty("completeThreshold", ""), 5);
    completeThreshold
        .setToolTipText("Time, in hours, that is waited before aggregations may start if data in complete. Default: 1 (hours)");

    JLabel clb_l = new JLabel("Complete aggregation threshold");
    clb_l
        .setToolTipText("Time, in hours, that is waited before aggregations may start if data is complete. Default: 1 (hours)");
    parent.add(clb_l, c);
    ...
    lookback
        .setToolTipText("Time, in days, that the monitoring is performed into history from current day. Default: 7 (days)");
    ...
    l_if.setToolTipText("Are the handeld flags ignored.");
    parent.add(l_if, c);

    String[] method = { "false", "true" };
    ignoreflag = new JComboBox(method);
    ignoreflag.setToolTipText("Are the handeld flags ignored.");
  ...
  public String validate() {
    ...
    String cth = completeThreshold.getText();
    if (cth.length() > 0) {
      try {
        Double.parseDouble(completeThreshold.getText());
      } catch (NumberFormatException nfe) {
       error += "complete threshold: Not a number\n";
      }
    }
    return error;
  }
