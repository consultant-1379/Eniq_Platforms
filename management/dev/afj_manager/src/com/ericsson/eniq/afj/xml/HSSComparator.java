...
import static com.ericsson.eniq.afj.common.PropertyConstants.PROP_HSS;
...
/**
 * This class is used to compare the counters from the input MOM file with the database entries in the DC_E_HSS techpack 
 * and give a delta.
 * For HSS, we are only interested in adding new counters to existing meas types.
 * The MOM moc name to DB meas name mapping is defined in the properties file. 
 * @author esunbal
 *
 */
public class HSSComparator implements CompareInterface {
	  ...
	  private final Logger log = Logger.getLogger(this.getClass().getName());
		...
		public List<AFJMeasurementType> getMeasTypeDelta(final Object object) throws AFJException{
			...
			techPackName = PropertiesUtility.getProperty(PROP_HSS_NAME);
		...
		private List<AFJMeasurementType> generateDelta(final PM pmObject) throws AFJException{
			...
			for(MocType mocType: mocTypes){
				...
				mappedMocName = getMappedMocName(mocType.getName());
				if(mappedMocName == null){
					log.info("The MocName:"+mocType.getName()+" is not mapped in the properties file.");
					continue;
				}
				
				if(measTagsToCountersMap.containsKey(mappedMocName.toUpperCase())){
					// Existing measurement type. Check whether there are new counters.	
					measTypeNameTokens = tagsToMeasNameMap.get(mappedMocName.toUpperCase()).split(DATAFORMAT_DELIMITER);
					measTypeObject.setTypeName(measTypeNameTokens[MEASTYPE_TOKEN]);
					measTypeObject.setTypeNew(false);				
					counters = measTagsToCountersMap.get(mappedMocName.toUpperCase());				
		 ...
		 * @throws AFJException
		 */
		private String getMappedMocName(final String inputMoc) throws AFJException{
			String returnValue = null;
			final String propertyKey = PROP_HSS + inputMoc;
			returnValue = PropertiesUtility.getProperty(propertyKey);
			return returnValue;
		}
