...
/**
 * @author esunbal
 * Class to upgrade the BSS AFJ. Handles both new counters in existing meas types and new meas types.
 ...
 *
 */
public class BSSUpgrader implements TechPackUpgrader {
  private final Logger log = Logger.getLogger(this.getClass().getName());
	...
	public String upgrade(final AFJDelta delta)throws AFJException{
		...
		techPackVersion = commonUtils.getActiveTechPackVersion(delta.getTechPackName(), dwhrep);
		
		log.info("Active version of TP:"+techPackVersion);
		...
		log.info("Calling engine restart");
		try{
	 ...
	 * @throws AFJException
	 */
	private String upgradeNewMeasType(final List<AFJMeasurementType> mtList)throws AFJException{
		String returnValue = null;
		 ...
		 * 9.3) Populate the Transformation table - Done 
		...
		try{			
			for(AFJMeasurementType mType: mtList){
				...
				commonUpgradeUtil.populateDataFormat(mType); // Task 1
				commonUpgradeUtil.populateDefaultTags(mType); // Task 3
				commonUpgradeUtil.populateMeasurementTypeClass(mType);
				commonUpgradeUtil.populateMeasurmentType(mType); // Task 4			
				commonUpgradeUtil.populateMeasurementTable(mType); // Task 8				
				commonUpgradeUtil.popluateTransformer(mType); // Task 9	
				commonUpgradeUtil.popluateTransformation(mType);
				commonUpgradeUtil.populateInterfaceMeasurement(mType);				
				measName = techPackVersion + DATAFORMAT_DELIMITER + mType.getTypeName();
	 ...
	 * @throws AFJException
	 */
	private String upgradeNewCountersInExistingMeasType(final List<AFJMeasurementType> mtList)throws AFJException{
