 ...
 *
 */
public class STNComparator implements CompareInterface {
	...
	private static final String[] validType = {"CC","SI", "GAUGE", "DER", "IDENTITY"};
	
	private static final String[] validSubType = {"MIN", "MAX", "LAST_UPDATE", "RAW", "MEAN", "SUM", ""};
	
	private static final String[] validResultType = {"decimal", "integer", "float", "byte", "int", "smallInt", "unsignedInt", "long", "double", "dateTime","date", "time", "boolean"};
	...
	public List<AFJMeasurementType> getMeasTypeDelta(final Object object) throws AFJException{
		...
		// Step 1: Preprocess the xml file
		// Step 2: Initialise all the datamembers needed for the class
		// Step 3: Generate the delta
		// Step 4: Return the delta result 
		...
		techPackName = PropertiesUtility.getProperty(PROP_STN_NAME);
	...
	private List<AFJMeasurementType> generateDelta(final PM pmObject) throws AFJException{
		...
		for(MocType mocType: mocTypes){
			...
			if(!measTagsToCountersMap.containsKey(mocType.getName().toUpperCase())){
				...
				measTypeObject.setDescription(mocType.getDescription());
				...
				// Populate and add the measTypeObject only if it has 1 counter
...
//				measTypeObject.setTypeName(tagsToMeasNameMap.get(mocType.getName().toUpperCase()));
	...
	private boolean sybaseNameCheck(final String name){
		...
		if(name.equalsIgnoreCase("ML-PPP")){
			log.info("ML-PPP is valid as per existing ENIQ database. Returning true.");			
			return true;
		}
