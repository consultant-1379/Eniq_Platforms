 ...
 *
 */
public class MTASComparatorTest {
	
	private static MTASComparator objectUnderTest;
	...
	public static void setUpBeforeClass() throws Exception {
		...
		statement.execute("insert into TPActivation (TECHPACK_NAME,STATUS,VERSIONID,TYPE,MODIFIED) values ('DC_E_MTAS','ACTIVE','DC_E_MTAS:((2))','PM',1)");
		...
		statement.execute("insert into DefaultTags (TAGID,DATAFORMATID,DESCRIPTION) values ('MtasQos','DC_E_MTAS:((2)):DC_E_MTAS_MTASQOS:mdc','Default tags for DC_E_MTAS_MTASQOS in DC_E_MTAS:((2)) with format mdc.')");
		statement.execute("insert into MeasurementCounter (TYPEID,DATANAME) values ('DC_E_MTAS:((2)):DC_E_MTAS_MTASQOS','MtasFuncInitOrigSessNOkE')");
		statement.execute("insert into MeasurementCounter (TYPEID,DATANAME) values ('DC_E_MTAS:((2)):DC_E_MTAS_MTASQOS','MtasFuncInitOrigSessNOkI')");
		statement.execute("insert into MeasurementCounter (TYPEID,DATANAME) values ('DC_E_MTAS:((2)):DC_E_MTAS_MTASQOS','MtasFuncInitOrigSessOk')");
		statement.execute("insert into MeasurementCounter (TYPEID,DATANAME) values ('DC_E_MTAS:((2)):DC_E_MTAS_MTASQOS','MtasFuncInitTermSessNOkE')");
	...
	public void setUp() throws Exception {
		  ...
		  objectUnderTest = new MTASComparator();
	...
	public void testGetMeasTypeDelta() throws Exception{	
		...
		ct1.setMeasurementName("MtasFuncInitOrigSessNOkE");
		ct1.setCollectionMethod(cm);
		ct1.setMeasurementResult(mr);
		final CounterType ct2 = new CounterType();
		ct2.setMeasurementName("MtasFuncInitOrigSessNOkI");
	...
	public void testCreateCounterObject() {
		...
		ct.setMeasurementName("MtasFuncInitOrigSessNOkI");
		ct.setMeasurementType("MtasFuncInitOrigSessNOkI");
	...
	public void testPreProcessInput() throws Exception{
		...
		ct5.setMeasurementName("MtasFuncInitOrigSessNOkESuccess");
		...
		final String expectedOuput = "\nThe character length exceeds Sybase IQ limitation of 128:cscfACABackupabcdefgcscfACABackupabcdefgcscfACABackupabcdefgcscfACABackupabcdefgcscfACABackupabcdefgcscfACABackupabcdefgcscfACABackupabcdefg" +
				"\nDuplicate counter name specification:MtasFuncInitOrigSessNOkE in the moc:MtasQos" +
				"\nInvalid counter name:" +
				"\nMtasFuncInitOrigSessNOkE. Name must begin with a letter (A through Z), underscore (_), at sign (@), dollar sign ($), or pound sign (#)" +
				"\nDuplicate moc name specification:MtasQos\nInvalid type:abc in counter:MtasFuncInitOrigSessNOkESuccess of moc:MtasQos" +
				"\nInvalid subType:def in counter:MtasFuncInitOrigSessNOkESuccess of moc:MtasQos" +
				"\nInvalid resultType:xyz in counter:MtasFuncInitOrigSessNOkESuccess of moc:MtasQos";	
