...
import com.ericsson.eniq.afj.xml.IMSComparator;
 ...
 *
 */
public class IMSCompartorTest {
	
	private static IMSComparator objectUnderTest;
	...
	public static void setUpBeforeClass() throws Exception {
		...
		statement.execute("insert into TPActivation (TECHPACK_NAME,STATUS,VERSIONID,TYPE,MODIFIED) values ('DC_E_IMS','ACTIVE','DC_E_IMS:((2))','PM',1)");
		...
		statement.execute("insert into DefaultTags (TAGID,DATAFORMATID,DESCRIPTION) values ('Cscf','DC_E_IMS:((2)):DC_E_IMS_CSCF:mdc','Default tags for DC_E_IMS_CSCF in DC_E_IMS:((2)) with format mdc.')");
		statement.execute("insert into MeasurementCounter (TYPEID,DATANAME) values ('DC_E_IMS:((2)):DC_E_IMS_CSCF','cscfACR')");
		statement.execute("insert into MeasurementCounter (TYPEID,DATANAME) values ('DC_E_IMS:((2)):DC_E_IMS_CSCF','cscfCxPullIdentitiesDontMatch')");
		statement.execute("insert into MeasurementCounter (TYPEID,DATANAME) values ('DC_E_IMS:((2)):DC_E_IMS_CSCF','cscfACABackup')");
		statement.execute("insert into MeasurementCounter (TYPEID,DATANAME) values ('DC_E_IMS:((2)):DC_E_IMS_CSCF','cscfACASuccess')");
	...
	public void setUp() throws Exception {
		  ...
		  objectUnderTest = new IMSComparator();
	...
	public void testCreateCounterObject() {
		...
		ct.setMeasurementName("cscfACR");
		ct.setMeasurementType("cscfACR");
	...
	public void testPreProcessInput() throws Exception{
		...
		ct2.setMeasurementResult(mr);
		final CounterType ct3 = new CounterType();
		ct3.setMeasurementName("cscfACR");
		...
		ct5.setMeasurementName("cscfACASuccess");
		...
		final String expectedOuput = "\nThe character length exceeds Sybase IQ limitation of 128:cscfACABackupabcdefgcscfACABackupabcdefgcscfACABackupabcdefgcscfACABackupabcdefgcscfACABackupabcdefgcscfACABackupabcdefgcscfACABackupabcdefg\nDuplicate counter name specification:cscfACR in the moc:Cscf" +
				"\nInvalid counter name:" +
				"\ncscfACR. Name must begin with a letter (A through Z), underscore (_), at sign (@), dollar sign ($), or pound sign (#)" +
				"\nDuplicate moc name specification:Cscf" +
				"\nInvalid type:abc in counter:cscfACASuccess of moc:Cscf" +
				"\nInvalid subType:def in counter:cscfACASuccess of moc:Cscf" +
				"\nInvalid resultType:xyz in counter:cscfACASuccess of moc:Cscf";	
