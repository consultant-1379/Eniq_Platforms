...
IF(SELECT COUNT(*) FROM sys.sysevent WHERE event_name = 'check_dwhdb_size_recover_check') > 0 THEN
    DROP EVENT check_dwhdb_size_recover_check;
END IF;
...
BEGIN
	...
	ALTER EVENT check_dwhdb_size_recover_check ENABLE;
    ELSE
...
CREATE EVENT check_dwhdb_size_recover_check
    SCHEDULE sched_check_dwhdb_size_recover_check START TIME '00:00 AM' EVERY 15 MINUTES
    DISABLE
HANDLER
BEGIN
    DECLARE mainspace_used_percentage INT;
    ...
    IF (90.0 > mainspace_used_percentage) THEN
		CALL xp_cmdshell('echo ' || logged_datetime || ' Warning: DWHDB is recoverying DB ! ConnCount: ' || conncount || ' MainSpaceUsed: ' || mainspace_used_percentage ||'% OtherVersions: ' || otherversions || ' >> /eniq/log/sw_log/iq/dwhdb_usage.log');
		CALL xp_cmdshell('engine -e changeProfile Normal "Warning: DWHDB is recoverying DB. Engine will be back to Normal."');
		ALTER EVENT check_dwhdb_size_recover_check DISABLE;
    ELSE
		CALL xp_cmdshell('echo ' || logged_datetime || ' SEVERE: DWHDB is not recovered ! ConnCount: ' || conncount || ' MainSpaceUsed: ' || mainspace_used_percentage ||'% OtherVersions: ' || otherversions || ' >> /eniq/log/sw_log/iq/dwhdb_usage.log')
