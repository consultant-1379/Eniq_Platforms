...
begin
	declare local temporary table iq_connTable (
		Number			unsigned bigint	not null,
		IQconnID		unsigned bigint	not null,
		IQCmdType		char(32)	not null,
		LastIQCmdTime		datetime	not null,
		IQCursors		unsigned int	not null,
		LowestIQCursorState	char(16)	not null,
		IQthreads		unsigned int	not null,
		TxnID			unsigned bigint	not null,
		ConnCreateTime		datetime	not null,
		TempWorkSpace		unsigned bigint	not null,
		TempTableSpace		unsigned bigint	not null,
		satoiq_count		unsigned bigint	not null,
		iqtosa_count		unsigned bigint	not null,
	...
	for loop1 as cursor1 cursor for
	    ...
	    and		i.DbRemote = 0						-- Not a dbremote connection.
