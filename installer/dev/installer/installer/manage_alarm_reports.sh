...
get_active_reports()
{
	...
	$DBISQL -nogui -c "eng=repdb;links=tcpip{host=repdb;port=2641};uid=${DWHREPusername};pwd=${DWHREPPassword}" "select REPORTID from AlarmReport where STATUS='ACTIVE';OUTPUT TO ${INSTALLER_DIR}/tmp_alarm_report.txt" >/dev/null
	if [ $? -ne "0" ] ; then
...
reports()
{
	if [[ "${1}" == "enable" ]] ; then
		...
		$DBISQL -nogui -c "eng=repdb;links=tcpip{host=repdb;port=2641};uid=${DWHREPusername};pwd=${DWHREPPassword}" /eniq/sw/installer/temp_alarm_reports.sql >/dev/null
		if [ $? -ne 0 ] ; then
