...
public class ActivateInterface extends CommonTask {
	...
	private void checkNetAnFeature() throws SQLException, FileNotFoundException, IOException {
		...
		if(this.activatedInterfaceName.toLowerCase().contains("vowifi") || this.activatedInterfaceName.toLowerCase().contains("volte") || 
			...
			ResultSet result = this.etlrepRockFactory.getConnection()
					.createStatement()
					.executeQuery("SELECT action_contents_01 FROM META_TRANSFER_ACTIONS WHERE ACTION_TYPE = 'Parse' AND COLLECTION_SET_ID = "
			...
			String path = properties.getProperty("inDir");
			path = path.replace("${PMDATA_DIR}", System.getenv().get("PMDATA_DIR"));
			
			this.createDataFileForNetAn(path);
		}
	...
	private void createDataFileForNetAn(String path) {
		...
		try{
			RemoteExecutor.executeComandSshKey("dcuser", "engine", "mkdir -p "+path.trim().substring(0, path.length()-1));
